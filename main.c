...
const uint32_t patch_stub_runtime_offset = patch_stub_offset - HEADER_SIZE;  // = 0x9E2
...
{
    // rel = dest_runtime - site_runtime - 2
    ...
    printf("%s: bsr.w at file offset $%X to stub at file offset $%X (rel %d / 0x%04X)\n",
...
int main(int argc, char **argv)
{
    ...
    if (argc != 3) {
        fprintf(stderr, "Usage: %s original.sys patched.sys\r\n", argv[0]);
        return 1;
    }
    ...
    if (!data) { perror("malloc"); fclose(fin); return 1; }

    fread(data, 1, insize, fin);
    fclose(fin);

    if (read_be16(data) != 0x601A) {
        fprintf(stderr, "Not a GEMDOS executable file\n");
        free(data);
        return 1;
    }

    if (patch_stub_offset + sizeof(patch_stub_code) > (uint32_t)insize) {
        fprintf(stderr, "Patch stub out of bounds\n");
        free(data);
        return 1;
    }
